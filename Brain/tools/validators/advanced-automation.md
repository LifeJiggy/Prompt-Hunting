# Advanced Automation — Input Validation Reference

**Domain**: Advanced Automation (Scanning & Automated Workflows)
**Validator Version**: 1.0.0
**Last Updated**: 2026-06-26
**Purpose**: Define input validation rules, JSON schemas, sanitization routines, type coercion, custom validators, and error handling for all scanning and automation-related inputs across the Advanced-Automation domain.
...
The Advanced Automation validator enforces strict input validation for every automation workflow in the `Advanced-Automation/` directory. Each file defines a scanning or automation technique — from subdomain enumeration to workflow orchestration — and accepts structured inputs that must be validated before execution. This validator ensures:
...
```json
{
  ...
  "properties": {
    "domain": {
      ...
      "description": "Must match the registered domain identifier"
    },
    "target": {
      "$ref": "#/definitions/ScanTarget"
    },
    "tool": {
      "$ref": "#/definitions/AutomationTool"
    },
    "parameters": {
  ...
  "definitions": {
    "OutputConfig": {
      ...
      "properties": {
        "format": {
          "type": "string",
          "enum": ["json", "csv", "xml", "text", "markdown", "html"]
        },
        "destination": {
          "type": "string",
          "maxLength": 4096
...
def validate_tool(input_data):
    ...
    allowed_tools = [
        ...
        "katana", "waybackurls", "gau", "katana", "amass", "sublist3r",
...
def sanitize_output_config(output):
    output["format"] = output.get("format", "json").lower()
    if output["format"] not in ("json", "csv", "xml", "text", "markdown", "html"):
        output["format"] = "json"
    if "destination" in output:
        output["destination"] = re.sub(r'[^\w\-\/\.\~]', '', output["destination"])
        output["destination"] = output["destination"][:4096]
    return output
```
...
| Version | Date | Change |
|---------|------|--------|
| 1.0.0 | 2026-06-26 | Initial validation reference for Advanced Automation domain |
